...
class NetbrainAPI(requests.Session):

    def __init__(self):
        super(NetbrainAPI, self).__init__()
        self.base_url = helpers.base_url(host=NETBRAIN_HOST)
        self.headers.update({'Content-Type': 'application/json', 'Accept': 'application/json'})
        self.verify = False

    def login(self):
        response = self.post(url='Session', data=SESSION_BODY)
        self.headers.update({'token': response['token']})

    def set_domain(self):
        self.put(url='Session/CurrentDomain', data=DOMAIN_BODY)

    def request(self, method, path, data=None, **kwargs):
        """ Extends base class method to handle DNA Center JSON data"""
        url = self.base_url + path
        data = json.dumps(data).encode('utf-8') if data is not None else None
        response = super(NetbrainAPI, self).request(method, url, data=data, **kwargs)
