...
class DnacAPI(requests.Session):

    def __init__(self):
        super(DnacAPI, self).__init__()
        self.base_url = helpers.base_url(host=DNAC_HOST)
        self.headers.update({'Content-Type': 'application/json'})
        self.verify = False

    def login(self, username, passwd):
        response = self.post(url='api/system/v1/auth/token', auth=(username, passwd))
    ...
    def request(self, method, path, data=None, **kwargs):
        ...
        response = super(DnacAPI, self).request(method, url, data=data, **kwargs)
